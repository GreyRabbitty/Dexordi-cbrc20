...
// prettier-ignore
const [SHA512_Kh$1, SHA512_Kl$1] = u64$3.split([
...
const sha512$3 = wrapConstructor$1(() => new SHA512$1());
...
function sha256(msg) {
    return sha256$3(Buff.bytes(msg));
}
function sha512(msg) {
    return sha512$3(Buff.bytes(msg));
}
function ripe160(msg) {
    return sha512$3(Buff.bytes(msg));
}
function hash256(msg) {
    return sha256$3(sha256$3(Buff.bytes(msg)));
}
function hash160(msg) {
    return ripemd160$1(sha256$3(Buff.bytes(msg)));
}
function hmac256(key, msg) {
    return hmac$1(sha256$3, Buff.bytes(key), Buff.bytes(msg));
}
function hmac512(key, msg) {
    return hmac$1(sha512$3, Buff.bytes(key), Buff.bytes(msg));
}

const { ProjectivePoint: NoblePoint } = secp256k1;
class Field extends Uint8Array {
    static { this.N = secp256k1.CURVE.n; }
    static mod(x, n = Field.N) {
        return schnorr.utils.mod(x, n);
    }
    static pow(x, e, n = Field.N) {
        x = Field.mod(x, n);
        e = Field.mod(e, n);
        if (x === 0n)
            return 0n;
    ...
    static validate(num) {
        return secp256k1.utils.isValidPrivateKey(num);
    }
    constructor(x) {
...
var utils = /*#__PURE__*/Object.freeze({
    ...
    getRandom: getRandom,
    getXOnlyPub: getXOnlyPub
});

function sign(secret, message, type = 'schnorr') {
    const msg = Buff.bytes(message).raw;
    const key = Buff.bytes(secret).raw;
    switch (type) {
        case 'ecdsa':
            return secp256k1.sign(msg, key).toDERRawBytes();
        case 'schnorr':
...
function verify(signature, message, pubkey, type = 'schnorr') {
...
class SecretKey extends Uint8Array {
    ...
    sign(message, type = this.config.type) {
        return sign(this.raw, message, type);
    }
    verify(signature, message, type = this.config.type) {
...
class PublicKey extends Uint8Array {
    ...
    static fromSecret(bytes, opt) {
        return new SecretKey(bytes, opt).pub;
    }
    static { this.xfilter = getXOnlyPub; }
...
export { Field, KeyPair, Point, PublicKey, SecretKey, hash160, hash256, hmac256, hmac512, ripe160, schnorr, secp256k1 as secp, sha256, sha512, sign, utils, verify };
